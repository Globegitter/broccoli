...
function broccoliCLI () {
  ...
  program.command('build <target>')
    ...
    .option('--force', 'Allow existing files to be overwritten', false)
    .option('--watch', 'watch for any changes and update the output directory')
    .action(function(outputDir, options) {
      actionPerformed = true
      var builder = getBuilder()
      if(options.watch) {
        broccoli.server.watch(builder, outputDir, options)
      } else {
        builder.build()
        .then(function (hash) {
          ...
          try {
            if(options.force) {
              copyDereferenceSync(dir, outputDir, true)
            } else {
